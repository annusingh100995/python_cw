...
'''Setting up global paramters'''

np.random.seed(1000)

# Start and end time (in milliseconds) for the simulation
tmin = 0.0
tmax = 50.0
...
'''Defining a class current that defines the whole system. The input curernt , the step and consant current
the equations and the model equations'''

class Current:

''' This  method takes in the input current as a float and raises error if the input s not a float type'''
...
'''This method sets the constant current type , user can change according to their experiment'''
...
'''Defining the function in the deffential equations'''
def alpha_n(Vm):
...
'''Defining the initla states of the paramenters '''
...
'''Defining the differential equations for constant current'''
# Compute derivatives
...
'''Defining the differential equations for step current'''
...
def main():
    ...
    '''Defining the inital parametres'''
    current = Current()
    Simulation_current = []
    time_step = 0
    current_mag = 0
    ...
    '''Taking input form the user to select type of current'''
    print(""" ======CURRENT MENU=======
                      1. Constant Current 
                      2. Step Current
                      """)
